<x-layouts.app>
    <x-slot:title>{{ $taskList->name }} - Task Manager</x-slot:title>
        ...
        >
            <x-slot:content>
                ...
                <div class="mt-4 flex items-center gap-2 text-sm text-gray-600">
                    <a href="{{ route('tasks.index') }}" class="hover:text-gray-900 transition-colors">
                        전체 할 일
                    </a>
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
                    <span class="font-medium text-gray-900">{{ $taskList->name }}</span>
                </div>
    ...
    <div class="space-y-2 mb-20">
        ...
        @else
            ...
            <div class="p-8 text-center rounded-lg border border-gray-200 bg-white shadow-sm">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900">이 목록에 할 일이 없습니다</h3>
    ...
    <div id="quick-add-task-bar"
        ...
        <form id="quick-add-task-form" class="flex items-center gap-3" data-task-list-id="{{ $taskList->id }}">
    ...
    @push('scripts')
        @vite(['resources/js/tasks/taskSlideOver.js', 'resources/js/tasks/quickAddTask.js', 'resources/js/task-lists/editTaskListTitle.js'])
    @endpush
</x-layouts.app>
